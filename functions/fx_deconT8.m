function [DATA_f] = fx_deconT8(DATA,dt,lf,mu,f0);
...
% Surpress Warning if Matrix becomes Singular
    % This occurs if time-domain traces are zero-padded
warning('off','MATLAB:singularMatrix');

nx = length(x);
% Roughening Matrix for Higher Order Tikhonov Regularization
L = eye(lf); % Zeroth Order
% Introduces Polarity Reversal
% L = -gallery('triw',lf,-1,1); % First Order
% L = gallery('tridiag',lf,1,-2,1);% Second Order
...
% backward ar-modeling
   ...
   % B is the correlation matrix of the signal.
   B = M'*M;  beta = diag(B).*mu/100;

   % a are the optimal filter coefficients
   ab = (B + (beta*ones(1,length(beta))).*L)\M'*y;
   temp = M*ab;
   temp = [temp;zeros(lf,1)];
   yb = temp;

% foreward AR-modeling
   y  = x(lf+1:nx,1);
   C  = x(lf:nx-1,1);
   R = flipud(x(1:lf,1));
   M = toeplitz(C,R);
   ...
   af = (B + (beta*ones(1,length(beta))).*L)\M'*y;
