function [pickX,pickT] = polarPickerGetFrame( Radar, snap, nHorizon, isZoom )
...
%                 [~,theta,thetaBin] = histcounts(MapT(:,1),2.*look+1);
        ...
        isWriteVideo = 1;
        if isWriteVideo
            v = VideoWriter('PolarPickerFastStartEnd.avi');
            v.FrameRate = 24;
            open(v);
            for ii = 1:3
                for jj = -10:ff+12
                    if jj>ff
                        writeVideo(v,Frame(ff));
                    elseif jj<2
                        writeVideo(v,Frame(ff));
                    else
